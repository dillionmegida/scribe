...
ipcMain.handle('transcribe', async (event, projectId: string) => {
  ...
  try {
    ...
    event.sender.send('transcribe-progress', { projectId, step: 'Converting audio…' });
    ...
    event.sender.send('transcribe-progress', { projectId, step: 'Transcribing with Whisper…' });
    ...
    if (isOpenAIWhisper) {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        let errOut = '';
        proc.stderr.on('data', d => errOut += d.toString());
        proc.on('close', code => code === 0 ? resolve() : reject(new Error(`whisper failed: ${errOut.slice(-300)}`)));
      });
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        let errOut = '';
        proc.stderr.on('data', d => errOut += d.toString());
        proc.on('close', code => code === 0 ? resolve() : reject(new Error(`whisper.cpp failed: ${errOut.slice(-300)}`)));
      });
